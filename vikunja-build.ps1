...
if ($Release) {
    ...
    if (-not (Get-Command gh -ErrorAction SilentlyContinue)) {
        ...
        $releaseNotes = "Custom build $tag`n`nPatched $patched files ($replaced replaced, $newFiles new)`nBuild time: $buildSec sec"
        ...
        # Create release and upload tar
        gh release create $tag $latestTar --title $releaseName --notes $releaseNotes --latest 2>&1 | Out-String | ForEach-Object {
            if ($_.Trim()) { Write-Host "  $_" -ForegroundColor DarkGray }
        }

        # If release already exists, just upload the asset
        if ($LASTEXITCODE -ne 0) {
            Write-Host "  Release exists, uploading asset..." -ForegroundColor DarkGray
            gh release upload $tag $latestTar --clobber 2>&1 | Out-Null
        }
        ...
        if ($releaseOK) {
            Write-Host "  Release OK: $tag ($sizeMB MB)" -ForegroundColor Green
        } else {
            Write-Host "  Release failed (check gh auth status)" -ForegroundColor DarkYellow
            Write-Host "  Manual: gh release create $tag $latestTar" -ForegroundColor DarkYellow
        }
...
if (-not $Deploy) {
    ...
    Write-Host ""
    if ($cancelled) { Write-Host "  Cancelled." -ForegroundColor Yellow; exit 0 }
