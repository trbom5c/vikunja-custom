// Gantt Dependency Arrow Tuner v5
// Adds exit/entry direction for stepped paths (right/bottom/top)
...
(function() {
  ...
  const config = {
    ...
    // Stepped
    exitDir: 'bottom',       // 'right' | 'bottom'
    entryDir: 'left',        // 'left' | 'top'
    exitLength: 30,
    entryLength: 20,
    cornerRadius: 11,
    // General
  ...
  panel.innerHTML = `
    ...
    <h3 id="tuner-drag-handle">🎯 Arrow Tuner v5</h3>
    ...
    <div class="row">
      <label>Opacity</label>
      <input type="range" min="0.1" max="1" step="0.05" value="0.5" data-key="opacity">
    ...
    <div class="section mode-panel active" id="panel-stepped">
      ...
      <div class="hint">Which edge of the source bar the line exits from</div>
      <div class="row">
      ...
      <div class="hint">Which edge of the target bar the line arrives at</div>
      <div class="row">
        <label>Exit Length</label>
        <input type="range" min="5" max="120" step="5" value="30" data-key="exitLength">
        <span class="val">30</span>
      </div>
      <div class="hint">Distance from bar before first turn</div>
      <div class="row">
        <label>Entry Length</label>
        <input type="range" min="5" max="120" step="5" value="20" data-key="entryLength">
        <span class="val">20</span>
      </div>
      <div class="hint">Distance from target before final turn</div>
      <div class="row">
        <label>Corner Radius</label>
        <input type="range" min="0" max="20" step="1" value="11" data-key="cornerRadius">
  ...
  function captureOriginals() {
    ...
    svg.querySelectorAll('path[class="dependency-arrow"], path[marker-end]').forEach(path => {
      ...
      if (b) {
        // sx,sy = right edge center of source bar; tx,ty = left edge center of target
  ...
  /**
   * Build a stepped path with configurable exit/entry directions.
   *
   * Exit from RIGHT:  sx,sy → right by exitLength → vertical to target row → right to target
   * Exit from BOTTOM: sx,sy → adjust to bottom of bar → down by exitLength → right to target col → up/down to target
   *
   * Entry from LEFT:  → approach from left by entryLength → arrive at tx,ty
   * Entry from TOP:   → approach from above by entryLength → arrive at tx,ty
   */
  function buildSteppedPath(sx, sy, tx, ty, barWidth) {
    const r = config.pathMode === 'stepRounded' ? config.cornerRadius : 0;
    const ex = config.exitLength;
    const en = config.entryLength;

    // Compute actual start/end points based on exit/entry direction
    let startX, startY, endX, endY;

    if (config.exitDir === 'bottom') {
      // Exit from bottom center of source bar (sx is right edge, so center is sx - barWidth/2)
      startX = sx - (barWidth || 0) / 2;
      startY = sy + BAR_HEIGHT / 2; // bottom of bar
    } else {
      startX = sx;     // right edge
      startY = sy;     // center
    }

    if (config.entryDir === 'top') {
      endX = tx + (barWidth || 0) / 2; // rough center
      endY = ty - BAR_HEIGHT / 2;  // top of bar
    } else {
      endX = tx;   // left edge
      endY = ty;   // center
    }

    // Now build the path based on directions
    if (config.exitDir === 'bottom' && config.entryDir === 'left') {
      // Down from source → right to align with target approach → right into target
      const dropY = startY + ex;                    // how far down before turning right
      const approachX = endX - en;                  // where to turn for final approach
      return buildRoundedPath([
        [startX, startY],    // start at bottom of source
        [startX, dropY],     // go down
        [approachX, dropY],  // go right to approach column
        [approachX, endY],   // go up/down to target row
        [endX, endY],        // enter target from left
      ], r);
    }

    if (config.exitDir === 'bottom' && config.entryDir === 'top') {
      const dropY = startY + ex;
      return buildRoundedPath([
        [startX, startY],
        [startX, dropY],
        [endX, dropY],
    ...
    if (config.exitDir === 'right' && config.entryDir === 'left') {
      const turnX = startX + ex;
      const approachX = endX - en;
      const midY = (startY + endY) / 2;

      if (approachX <= turnX + 5) {
        // Not enough room for mid-horizontal, simple L
        return buildRoundedPath([
          [startX, startY],
          [turnX, startY],
          [turnX, endY],
  ...
  /**
   * Build an SVG path through waypoints with optional rounded corners.
   * waypoints: [[x,y], [x,y], ...]
   */
  function buildRoundedPath(pts, r) {
    ...
    for (let i = 1; i < pts.length - 1; i++) {
      ...
      const startRX = curr[0] - (dx1/len1) * cr;
      const startRY = curr[1] - (dy1/len1) * cr;
      const endRX = curr[0] + (dx2/len2) * cr;
      const endRY = curr[1] + (dy2/len2) * cr;

      d += ` L ${startRX} ${startRY}`;
      d += ` Q ${curr[0]} ${curr[1]}, ${endRX} ${endRY}`;
    }
  ...
  function updateDiagram() {
    ...
    if (!curve) return;
    // Diagram: source bar right=65, cy=18; target bar left=230, cy=52
    if (config.pathMode === 'bezier') {
      const { path } = buildBezierPath(65, 18, 230, 52);
      curve.setAttribute('d', path);
    } else {
      const saved = {...config};
      // Scale for mini diagram
      config.exitLength = config.exitLength * 0.3;
      config.entryLength = config.entryLength * 0.3;
      config.cornerRadius = config.cornerRadius * 0.4;
      curve.setAttribute('d', buildSteppedPath(65, 18, 230, 52, 60));
  ...
  function redraw() {
    ...
    arrowPaths.forEach((pathEl, i) => {
      ...
      if (orig && orig.type === 'bezier') {
        ...
        // Estimate bar width from original path data (rough: assume bars are ~day-width minimum)
        const barWidth = 0; // We don't have this, stepped will use center offset
        ...
        if (config.pathMode === 'bezier') {
          ...
          pathEl.setAttribute('d', buildSteppedPath(sx, sy, tx, ty, barWidth));
          if (config.showControlPoints) {
            // Highlight waypoints by parsing the path
            const coords = pathEl.getAttribute('d').match(/[\d.-]+ [\d.-]+/g);
            if (coords) coords.forEach(c => {
              const [cx,cy] = c.split(' ').map(Number);
              addCircle(svg, cx, cy, 3, 'rgba(255,200,100,0.7)');
            });
          }
        }
      }
    ...
    updateDiagram();
    updateModePanels();
    setStatus(`✓ ${arrowPaths.length} arrows (${config.pathMode}, exit:${config.exitDir}, enter:${config.entryDir})`);
  }
  ...
  document.getElementById('tuner-reset').addEventListener('click', () => {
    Object.assign(config, {
      strokeWidth:1.5, dashArray:'2,2', opacity:0.5, pathMode:'stepRounded',
      cp1X:0.5, cp1Y:0, cp2X:0.5, cp2Y:0,
      exitDir:'bottom', entryDir:'left', exitLength:30, entryLength:20, cornerRadius:11,
      arrowSize:8, zIndex:5, showDots:false, dotRadius:3,
      showShadow:false, shadowWidth:4, shadowOpacity:0.2, showControlPoints:false, palette:'multi',
    });
    panel.querySelectorAll('input[type="range"]').forEach(input => {
  ...
  redraw();
  console.log('%c🎯 Arrow Tuner v5 — Exit/Entry direction controls', 'color: #5DA5DA; font-size: 14px;');
})();
