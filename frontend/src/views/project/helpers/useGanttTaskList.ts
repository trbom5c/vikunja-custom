			...
			if (movedEarlier && hasPredecessors) {
				...
				if (collides) {
					...
					if (earliest) {
						...
						if (shifted < today) {
							window.alert(`Note: this will move predecessor tasks to ${shifted.toLocaleDateString()}, which is in the past.`)
						}
					}
			...
			if (hasSuccessors) {
				if (movedEarlier) {
					...
					if (earliest) {
						const shifted = new Date(earliest.getTime() + deltaDays * 24 * 60 * 60 * 1000)
						if (shifted < today) {
							window.alert(`Note: this will move downstream tasks to ${shifted.toLocaleDateString()}, which is in the past.`)
						}
					}
				}
