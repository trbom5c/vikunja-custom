...
export interface UseGanttTaskListReturn {
	...
	onCascadePrompt: Ref<((info: {label: string, names: string, absDays: number, direction: string, accentColor: string, action: () => void, dismiss?: () => void, barId?: string}) => void) | null>
}
...
export function useGanttTaskList<F extends Filters>(
	...
	const onCascadePrompt = ref<((info: {label: string, names: string, absDays: number, direction: string, accentColor: string, action: () => void, dismiss?: () => void, barId?: string}) => void) | null>(null)
				...
				if (collides) {
					...
					if (promptStyle === 'modal') {
						...
						onCascadePrompt.value({
							...
							action: () => { cascadeShiftChain(predecessors!, deltaDays, 'follows'); clearCascadePreview(predPreviewId) },
						...
						warning(`Overlaps: ${predNames}`, [{
							...
							callback: () => { cascadeShiftChain(predecessors!, deltaDays, 'follows'); clearCascadePreview(predPreviewId) },
						}, {
							title: 'Dismiss',
			...
			if (hasSuccessors) {
				...
				if (promptStyle === 'modal') {
					...
					onCascadePrompt.value({
						...
						action: () => { cascadeShiftChain(successors!, deltaDays, 'precedes'); clearCascadePreview(succPreviewId) },
					...
					warning(`${succNames}`, [{
						...
						callback: () => { cascadeShiftChain(successors!, deltaDays, 'precedes'); clearCascadePreview(succPreviewId) },
					}, {
						title: 'Dismiss',
