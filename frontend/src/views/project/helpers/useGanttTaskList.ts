...
export interface CascadeBubbleInfo {
	label: string
	names: string
	absDays: number
	direction: string
	...
	// For individual mode: list of tasks to confirm one by one
	pendingTasks?: Array<{id: number, name: string, action: () => void}>
}
...
export interface UseGanttTaskListReturn {
	...
	onCascadePrompt: Ref<((info: CascadeBubbleInfo) => void) | null>
}
...
export function useGanttTaskList<F extends Filters>(
	...
	const onCascadePrompt = ref<((info: CascadeBubbleInfo) => void) | null>(null)
