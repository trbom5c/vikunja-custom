...
export interface UseGanttTaskListReturn {
	...
	cascadePreview: Ref<CascadePreview | null>
}
		...
		try {
			...
			if (movedEarlier && hasPredecessors) {
				...
				if (collides) {
					...
					cascadePreview.value = {taskIds: previewIds, deltaDays, direction: 'follows'}
					...
					if (promptStyle === 'modal') {
						...
						warning(`Overlaps: ${predNames}`, [{
							...
							callback: () => { cascadeShiftChain(predecessors!, deltaDays, 'follows'); clearCascadePreview() },
						}, {
							title: 'Dismiss',
							callback: clearCascadePreview,
			...
			if (hasSuccessors) {
				...
				cascadePreview.value = {taskIds: previewIds, deltaDays, direction: 'precedes'}
				...
				if (promptStyle === 'modal') {
					...
					warning(`${succNames}`, [{
						...
						callback: () => { cascadeShiftChain(successors!, deltaDays, 'precedes'); clearCascadePreview() },
					}, {
						title: 'Dismiss',
						callback: clearCascadePreview,
