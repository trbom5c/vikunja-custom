...
function cancelDrag() {
	if (dragConfirm.value?.phase === 'cascade') {
		// Skip cascade — just close, move already applied
		dragConfirm.value = null
	} else {
...
function showCascadeInBubble(info: {label: string, names: string, absDays: number, direction: string, accentColor: string, action: () => void, barId?: string}) {
	const containerEl = ganttContainer.value as HTMLElement | null
	// Try to anchor to the first affected downstream bar
