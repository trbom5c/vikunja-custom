		...
		<Transition name="bubble">
			<div
				...
				:style="{
					...
					borderColor: dragConfirm.cascadeAccentColor || 'var(--grey-700, #444)',
				}"
			>
				<!-- Move phase -->
				<template v-if="dragConfirm.phase === 'move'">
					<div class="drag-confirm-label">
						{{ dragConfirm.taskName }}
					</div>
					<div class="drag-confirm-detail">
						{{ dragConfirm.absDays }}d {{ dragConfirm.direction }}
					</div>
				</template>
				<!-- Cascade phase -->
				<template v-else>
					<div class="drag-confirm-cascade-icon" :style="{color: dragConfirm.cascadeAccentColor || '#7e7'}">
						↓
					</div>
					<div class="drag-confirm-label">
						{{ dragConfirm.cascadeNames }}
					</div>
					<div class="drag-confirm-detail">
						{{ dragConfirm.absDays }}d {{ dragConfirm.direction }}
					</div>
				...
				<div class="drag-confirm-actions">
					<button class="drag-confirm-btn confirm" @click="confirmDrag" title="Shift all">
						✓ All
					</button>
					<button class="drag-confirm-btn cancel" @click="cancelDrag" :title="dragConfirm.phase === 'cascade' ? 'Skip' : 'Cancel'">
						✕
					</button>
				</div>
			</div>
...
// Drag confirm bubble — handles both move confirm and cascade prompts
const dragConfirm = ref<{
	...
	originalEnd: Date
	currentDays: number
	// Phase: 'move' = confirm drag, 'cascade' = confirm cascade shift
	phase: 'move' | 'cascade'
...
function confirmDrag() {
	...
	if (dragConfirm.value.phase === 'move') {
		...
		// Apply the move — this triggers updateGanttTask which may emit cascade info
		updateGanttTask(barId, newStart, newEnd)

		// Clear the move phase — cascade phase will be set by the cascade handler if needed
		isDragging.value = false
		dragState.value = null

		// Wait briefly for cascade detection to populate the bubble
		setTimeout(() => {
			if (dragConfirm.value?.phase === 'move') {
				// No cascade was triggered — close bubble
				dragConfirm.value = null
			}
		}, 300)
	} else if (dragConfirm.value.phase === 'cascade') {
		// Execute the cascade shift
...
function cancelDrag() {
	if (dragConfirm.value?.phase === 'cascade') {
		// Dismiss cascade — just close bubble, move already applied
		dragConfirm.value = null
	} else {
...
if (props.onCascadePrompt) {
	props.onCascadePrompt.value = (info) => {
		if (dragConfirm.value) {
			...
			// No bubble open — create one anchored near bottom-left as fallback
			...
			dragConfirm.value = {
				...
				absDays: info.absDays,
				direction: info.direction,
				x: (containerEl?.scrollLeft || 0) + 200,
				y: (containerEl?.scrollTop || 0) + (containerEl?.clientHeight || 300) - 60,
