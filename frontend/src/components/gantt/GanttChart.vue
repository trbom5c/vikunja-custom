...
const props = withDefaults(defineProps<{
	...
	cascadePreview?: {taskIds: Set<number>, deltaDays: number, direction: string} | null
}>(), {
...
watch(
	() => props.cascadePreview,
	(preview) => {
		// Clean up previous preview
		const container = ganttContainer.value as HTMLElement | null
		if (!container) return

		// Remove old ghosts and pulses
		container.querySelectorAll('.cascade-ghost').forEach(el => el.remove())
		container.querySelectorAll('.cascade-pulse').forEach(el => el.classList.remove('cascade-pulse'))

		if (!preview) return

		const {taskIds, deltaDays} = preview
		const deltaPixels = deltaDays * DAY_WIDTH_PIXELS

		requestAnimationFrame(() => {
			for (const taskId of taskIds) {
				// Find the bar rect for this task
...
@keyframes bar-cascade-pulse {
	...
	50% {
		filter: brightness(1.4) drop-shadow(0 0 6px rgba(240, 173, 78, 0.6));
	}
}
